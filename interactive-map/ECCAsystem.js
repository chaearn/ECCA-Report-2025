...
async function _2(FileAttachment,d3)
{
  ...
  function cardHTML(d, chart, variant = "master") {
    ...
    const countries = variant === "master"
      ? String(d.country || "").split(",").map(c => c.trim()).filter(c => COUNTRY_ICON[c])
      : [];
  ...
  function makePanel(chart, index) {
    ...
    (function renderDetail() {
      ...
      const iconR = isSmallScreen() ? 9 : 12;    // 24x24 node icons, 18x18 on a phone
      ...
      // Blur via a real <filter>, not the CSS blur() shorthand: Safari does not
      // support CSS filter *functions* on SVG elements (only filter="url(#…)"),
      // so the shorthand silently did nothing there and the bands rendered as
      // hard concentric rings instead of a soft wash. Every drop-shadow() in
      // this file is on an HTML <img>, where Safari is fine — this contour was
      // the only CSS filter aimed at SVG, and the only thing that never blurred.
      // Blend stays on a separate outer group from the filtered one, since
      // WebKit is also unreliable when both sit on a single element.
      // CSS blur(N) === feGaussianBlur stdDeviation N, so the radius is unchanged.
      const blurId = `contour-blur-${chart.id.replace(/\W+/g, "-")}`;
      defs.append("filter")
        .attr("id", blurId)
        // Roomier than the -10%/120% default so a wide blur is not clipped.
        .attr("x", "-25%").attr("y", "-25%").attr("width", "150%").attr("height", "150%")
        .append("feGaussianBlur")
        .attr("in", "SourceGraphic")
        .attr("stdDeviation", base * 0.035);
      const contourBlend = detailGroup.append("g")
        .attr("transform", `translate(${cox},${coy})`)
        .style("pointer-events", "none").style("mix-blend-mode", "multiply");
      const contourG = contourBlend.append("g").attr("filter", `url(#${blurId})`);
      // NB: bandwidth is a fixed px setting while the grid it runs over (cHalf)
      // scales with the viewport, so on a phone the kernel spans ~55% of the
      // grid vs ~18% on desktop — a much smoother, flatter field. Scaling the
      // kernel down to match desktop's ratio was tried and is worse: it breaks
      // the glow into per-node speckle instead of one soft radial wash. The
      // wide kernel is what makes the wash read, so leave it alone.
      ...
      nodeSel.append("image").attr("href", d => d.icon).attr("xlink:href", d => d.icon)
        ...
        .attr("preserveAspectRatio", "xMidYMid meet");
      // The label column was base*0.16 — but `base` scales with the viewport, so
      // on a phone that is a ~22px (≈5ch) column and every label shreds into one
      // word per line, which is what made the sub-zones look so crammed. Label
      // width should not depend on the viewport at all: pin it to ~16ch of its
      // own font. Desktop was already ~16ch (66.8px), so this barely moves it.
      const detailLabelFont = isSmallScreen() ? 6 : settings.nodeLabelFontSize;
      const detailLabelW = detailLabelFont * 8.8; // ≈16ch of Poppins (~0.55em per ch)
      nodeSel.append("foreignObject").attr("class", "node-label-fo").attr("x", -detailLabelW / 2).attr("y", iconR)
        .attr("width", detailLabelW).attr("height", base * 0.1).style("overflow", "visible").style("pointer-events", "none")
        .append("xhtml:div").attr("class", "detail-node-label")
        .style("font-size", detailLabelFont + "px")   // matches the master node labels on desktop
      ...
      if (art) {
        detailControls = {
          ...
          setNodeArea(xf, yf) {
            ...
            // contourBlend, not contourG: the translate lives on the outer
            // (blend) group — the inner one only carries the blur.
            contourBlend.attr("transform", `translate(${nx - cHalf},${ny - cHalf})`);
  ...
  function overviewBBox() {
    // The detail maps sit far outside the overview, and the info cards are
    // parked up-left of their nodes. Both are invisible here but would still
    // dominate the measurement.
    return measuredBBox(zoomLayer, ".detail-maps, .node-card");
  }

  function computeOverviewTransform() {
    const bb = isSmallScreen() ? overviewBBox() : null;
    const k = bb
      ? Math.min(
          0.92,
          (viewportW - OVERVIEW_FIT_PAD * 2) / bb.width,
          (viewportH - OVERVIEW_FIT_PAD * 2) / bb.height
        )
      : 0.92;
  ...
  function refitOverview() {
    if (!isSmallScreen() || userHasZoomed || soloedIndex !== null) return;
  ...
  // The detail maps sit outside the master view and were kept out of sight
  // purely by being parked off-screen (see makePanel). That holds at the
  // desktop zoom, but the small-screen fit above zooms out far enough to drag
  // them into frame at the edges, so hide them outright unless a theme is
  // actually soloed. On desktop they were never visible here anyway, so this
  // changes nothing there — it just stops depending on the viewport to crop.
  function setDetailMapsVisible(on) {
    detailLayer.style("display", on ? null : "none");
  }
  setDetailMapsVisible(false);
  ...
  function detailFitTransform(index) {
    ...
    if (isSmallScreen() && p.detailGroup) {
      ...
      const bb = measuredBBox(p.detailGroup, ".detail-node-card, .node-label-fo");
      if (bb) {
  ...
  function soloPanel(index) {
    soloedIndex = index;
    setDetailMapsVisible(true); // before the fly-in, so the target is drawn en route
  ...
  function exitSolo() {
    soloedIndex = null;
    // Hide only once the fly-out has landed, or the detail map would vanish
    // mid-flight. The guard covers a re-solo interrupting the transition.
    svg.transition().duration(750).call(zoom.transform, overviewTransform)
      .on("end", () => { if (soloedIndex === null) setDetailMapsVisible(false); });
